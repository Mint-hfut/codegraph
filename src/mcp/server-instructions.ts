...
- **Project docs & knowledge** — README/markdown docs, agent skills (SKILL.md), slash commands (\`.claude/commands/*.md\`), agent memory (CLAUDE.md/AGENTS.md), Dockerfile/compose, CI workflows, package.json scripts — are in the graph too, as \`document\`/\`section\` nodes with \`references\` edges to the code they mention. For skills and commands the trigger condition ("Use when…"), allowed tools, argument hint, and model from the frontmatter are searchable, so "which skill handles X" / "what command takes a Y argument" resolve. A skill's whole bundle (its scripts and reference docs) is linked from its SKILL.md, and skills/memory living outside the project (e.g. \`~/.claude/skills\`, when configured as extra roots) appear under \`~extra/…\` paths. Binary assets (images/video/PDF) are findable by NAME, but their content is not indexed. Ask \`codegraph_explore\` the same way ("what does the README say about deployment", "which skill covers releases"); filter \`codegraph_search\` with \`kind: document\`.
